...
사용자별·데이터셋별로 분석 이력이 쌓여, 이전 분석을 언제든 재활용할 수 있습니다.

![인사이트가쌓이는모습](image/인사이트가쌓이는모습.png)

### 인사이트 기반 대시보드
축적된 인사이트를 `dataiku.insights.save_data()`로 Static Insight에 게시하고,  
NexusData Dashboard에 타일로 자동 추가합니다. 별도 대시보드 작업 없이 분석 결과가 바로 공유됩니다.

![인사이트기반대시보드](image/인사이트기반대시보드배포.png)

### 세션별 데이터 로그
사용자별·데이터셋별로 대화 히스토리가 Managed Folder(`nexusdata_charts/_history/`)에 자동 저장됩니다.  
세션이 바뀌어도 이전 분석 맥락을 불러와 이어서 분석할 수 있습니다.

![세션별데이터로그](image/세션별데이터로그.png)
